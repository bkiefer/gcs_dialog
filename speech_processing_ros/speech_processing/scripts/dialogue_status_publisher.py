...
from gcs_msgs.msg import DialogStatus
from threading import Thread
from SocketServer import ThreadingMixIn


class DialogStatusPublisher:
    local_address = (os.environ.get("DIA_IP"), 11115)
    BUFFER_SIZE = 20  # Usually 1024, but we need quick response

    def __init__(self):
        print("init bridge at " + str(self.local_address))
        self.publisher = rospy.Publisher(
            '/dialog_status', DialogStatus, queue_size=1)
...
if __name__ == '__main__':
    print("init ros")
    rospy.init_node('dialogStatus')
    bridge = DialogStatusPublisher()
    print("rospy.spin")
    rospy.spin()
