...
class RespeakerInterface(object):
    ...
    def __init__(self):
        ...
        self.set_led_trace()
        rospy.loginfo("Respeaker device initialized (Version: %s)" %
                      self.version)
    ...
    def set_led_color(self, r, g, b, a):
        self.pixel_ring.set_brightness(int(5 * a))
...
class RespeakerNode(object):
    def __init__(self):
        ...
        suppress_pyaudio_error = rospy.get_param(
        ...
        self.prev_doa = None
        # advertise
        self.pub_vad = rospy.Publisher(
            "is_speeching", Bool, queue_size=1, latch=True)
        self.pub_doa_raw = rospy.Publisher(
            "sound_direction", Int32, queue_size=1, latch=True)
        self.pub_doa = rospy.Publisher(
            "sound_localization", PoseStamped, queue_size=1, latch=True)
        self.pub_audio = rospy.Publisher("audio", AudioData, queue_size=10)
        self.pub_speech_audio = rospy.Publisher(
            "speech_audio", AudioData, queue_size=10)
        # init config
        self.config = None
        ...
        self.info_timer = rospy.Timer(rospy.Duration(1.0 / self.update_rate),
                                      self.on_timer)
        self.timer_led = None
        self.sub_led = rospy.Subscriber(
            "status_led", ColorRGBA, self.on_status_led)
        self.service = rospy.Service(
            "/mlt/mic_toggle_service", Trigger, self.on_toggle)

    def on_toggle(self, req):
        print("Toggle")
        self.respeaker_audio.toggle()
        response = TriggerResponse(True, "")
        return response
